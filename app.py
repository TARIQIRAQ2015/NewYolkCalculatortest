...
# اختيار اللغة
language = st.selectbox("Select Language", ["English", "Română"])
...
with col1:
    currency = st.selectbox(
        ...
        ["USD", "IQD"] if language == "English" else ["USD", "IQD"]
    )
...
if st.button(texts[language]["save_prices"], type="secondary"):
    try:
        ...
        st.error("Please enter valid numbers! ❗️" if language == "English" else "Vă rugăm să introduceți numere valide! ❗️")

# تحديث الأسعار بناءً على العملة
if currency == "Iraqi Dinar" or currency == "Dinar Irakian":
...
if calculation_type == texts[language]["chicken_profits"]:
    ...
    with col5:
        eggs = st.text_input(
            ...
            help="Enter the number of eggs (max 580)" if language == "English" else "Introduceți numărul de ouă (maxim 580)",
            key="eggs_input"
        )
    ...
    with col6:
        days = st.text_input(
            ...
            help="Enter the number of days (max 730)" if language == "English" else "Introduceți numărul de zile (maxim 730)",
            key="days_input"
        )
    ...
    if st.button(texts[language]["calculate_profits"], type="primary"):
        try:
            ...
            if eggs is None or days is None:
                st.error("Please enter all required values! ❗️" if language == "English" else "Vă rugăm să introduceți toate valorile necesare! ❗️")
            elif eggs > 580:
                st.error("Number of eggs should not exceed 580! ❗️" if language == "English" else "Numărul de ouă nu trebuie să depășească 580! ❗️")
            elif days > 730:
                st.error("Number of days should not exceed 730! ❗️" if language == "English" else "Numărul de zile nu trebuie să depășească 730! ❗️")
            else:
                # حساب النتائج
                ...
                # عرض النتائج كجدول
                st.success("Calculation successful! ✅" if language == "English" else "Calcul reușit! ✅")
    ...
    with col8:
        food = st.text_input(
            ...
            help="Enter the amount of food needed" if language == "English" else "Introduceți cantitatea de mâncare necesară",
            key="food_input"
        )
...
if st.button(texts[language]["reset"], type="secondary"):
    ...
    st.session_state.rewards = ""
    st.session_state.food = ""
    st.success("Reset successful! ✅" if language == "English" else "Resetare reușită! ✅")
